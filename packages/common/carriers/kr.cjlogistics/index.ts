...
import { DeliveryProgressStatusMap, TrackingInformation } from 'types';

const STATUS_MAP: DeliveryProgressStatusMap = {
...
async function getTrack(trackId: string) {
  ...
  const cookie =
    generalInfo.headers['set-cookie'] &&
    generalInfo.headers['set-cookie']
  ...
  const $ = load(generalInfo.data);

  const detailInfo = await axios.post(
  ...
  const informationTable = detailInfo.data.parcelResultMap.resultList;
  const progressTable = detailInfo.data.parcelDetailResultMap.resultList;
  ...
  if (informationTable.length !== 0) {
    shippingInformation.from = {
      ...
      time:
        progressTable.length !== 0 ? parseTime(progressTable[0].dTime) : null,
    };
